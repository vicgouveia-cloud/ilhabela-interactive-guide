(() => {
  ...
  function render(){
    ...
    grid.innerHTML=rows.map(({service,distance})=>{
      ...
      const image=service.image?`<img src="/${service.image.replace(/^\\//,'')}" alt="${service.name}" class="w-full h-full object-cover" loading="lazy">`:`<div class="w-full h-full flex items-center justify-center bg-surface-container"><span class="material-symbols-outlined text-primary/30 text-5xl">${service.category==='food'?'restaurant':'storefront'}</span></div>`;
