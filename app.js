...
function rerenderServicesPage() {
  const root = document.getElementById('services-page-grid');
  if (!root || typeof window.renderServicesPage !== 'function') return;
...
function setLanguage(lang, rerender = true) {
  ...
  if (rerender) {
    renderSpotsGrid(); renderGuides(); renderServicesDirectory(); rerenderServicesPage(); updateMapMarkers(); populateBookingGuides(); updateBookingEstimate();
