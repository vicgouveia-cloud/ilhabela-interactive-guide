...
function getLocalRecommendationTranslation(item) {
  ...
  if (item.translations && item.translations['pt']) {
    return item.translations['pt'];
  }
  return item;
}
...
function renderLocalRecommendations(spotId) {
  ...
  const spotTitle = spot ? (getSpotTranslation(spot).title || spot.name || 'atração') : 'atração';
  ...
  const whatsappLink = item => item.whatsapp ? `<a href="https://wa.me/55${item.whatsapp}?text=${encodeURIComponent(t('whatsappMsg'))}" target="_blank" rel="noopener noreferrer" class="flex-1 min-h-11 py-2.5 px-4 rounded-xl bg-[#25D366] hover:bg-[#1EBE5D] text-white text-[11px] font-bold shadow-md flex items-center justify-center gap-1.5 transition-colors"><span class="material-symbols-outlined text-[16px]">chat</span><span>${item.whatsappDisplay || 'WhatsApp'}</span></a>` : '';
  const siteLink = item => item.url ? `<a href="${item.url}" target="_blank" rel="noopener noreferrer" class="min-h-11 py-2.5 px-4 rounded-xl glass-panel text-on-surface-variant hover:text-primary text-[11px] font-bold border border-black/10 flex items-center justify-center gap-1.5 transition-colors"><span class="material-symbols-outlined text-[16px]">language</span><span>${t('accessSite')}</span></a>` : '';

  let html = '';
  if (recs.length) {
    html += `<section class="pt-6 border-t border-black/10 mt-6 space-y-4"><div><h3 class="text-lg md:text-xl font-extrabold text-primary font-heading uppercase tracking-wide">${t('servicesTitle').replace('{spot}', spotTitle)}</h3><p class="text-xs md:text-sm text-on-surface-variant mt-1">${t('servicesSubtitle')}</p></div><div class="grid grid-cols-1 gap-5">`;
    html += recs.map(recRaw => {
      const rec = { ...recRaw, ...getLocalRecommendationTranslation(recRaw) };
      return `<article class="glass-card rounded-2xl overflow-hidden flex flex-col md:flex-row border border-black/5 shadow-sm group"><div class="w-full md:w-2/5 h-48 md:h-auto relative shrink-0">${rec.image ? `<img src="${rec.image}" alt="${rec.name}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" decoding="async">` : `<div class="w-full h-full bg-surface-container flex flex-col items-center justify-center text-on-surface-variant/50"><span class="material-symbols-outlined text-[32px] mb-2">image</span><span class="text-[10px] uppercase font-bold tracking-wider">${t('pendingImage')}</span></div>`}<div class="absolute top-3 left-3 px-2.5 py-1 rounded-full bg-white/90 backdrop-blur-md text-[10px] font-extrabold text-primary uppercase shadow-sm">${rec.type}</div></div><div class="p-4 md:p-5 flex flex-col justify-center flex-1 space-y-3"><div><h4 class="text-lg font-bold text-primary font-heading leading-tight">${rec.name}</h4>${rec.tagline ? `<p class="text-xs font-semibold text-secondary mt-0.5">${rec.tagline}</p>` : ''}</div><p class="text-xs text-on-surface-variant leading-relaxed">${rec.description}</p>${rec.highlightTitle ? `<div class="bg-primary/5 rounded-lg p-2.5 border-l-2 border-primary"><span class="block text-[10px] font-bold text-primary uppercase mb-0.5">${rec.highlightTitle}</span><span class="text-xs font-semibold text-on-surface-variant">${rec.highlightDesc}</span></div>` : ''}${renderTags(rec.features || rec.tags)}${rec.alsoOffers?.length ? `<p class="text-[11px] text-on-surface-variant"><strong>${t('alsoOffers')}</strong> ${rec.alsoOffers.join('; ')}.</p>` : ''}<div class="flex flex-wrap gap-2 pt-2 mt-auto">${whatsappLink(rec)}${rec.instagram ? `<a href="https://instagram.com/${rec.instagram.replace('@','')}" target="_blank" rel="noopener noreferrer" class="min-h-11 py-2.5 px-4 rounded-xl glass-panel text-on-surface-variant hover:text-primary text-[11px] font-bold border border-black/10 flex items-center justify-center gap-1.5 transition-colors"><span>@</span><span>Instagram</span></a>` : ''}${siteLink(rec)}</div></div></article>`;
    }).join('');
    html += '</div></section>';
  }

  if (stays.length) {
    html += `<section class="pt-6 border-t border-black/10 mt-6 space-y-4"><div><h3 class="text-lg md:text-xl font-extrabold text-primary font-heading uppercase tracking-wide">${t('staysTitle').replace('{spot}', spotTitle)}</h3><p class="text-xs md:text-sm text-on-surface-variant mt-1">${t('staysSubtitle')}</p></div><div class="grid grid-cols-1 gap-5">`;
    html += stays.map(stayRaw => {
      const stay = { ...stayRaw, ...getLocalRecommendationTranslation(stayRaw) };
