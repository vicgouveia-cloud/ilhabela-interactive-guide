...
// One renderer powers both the Home preview and the dedicated /servicos/ page.
function initServicesPage() {
  ...
  const render = () => {
    ...
    filters.querySelectorAll('[data-service-page-category]').forEach(button => button.addEventListener('click', () => { category = button.dataset.servicePageCategory; render(); }));
    ...
    grid.innerHTML = rows.map(({ service, distance }) => {
      ...
      const image = service.image
        ? `<img src="/${service.image.replace(/^\//, '')}" alt="${service.name}" class="w-full h-full object-cover" loading="lazy" decoding="async">`
        : `<div class="w-full h-full flex items-center justify-center bg-surface-container/70"><span class="material-symbols-outlined text-primary/35 text-5xl">${service.category === 'food' ? 'restaurant' : 'storefront'}</span></div>`;
  ...
  document.getElementById('services-distance')?.addEventListener('change', render);
  render();
}
